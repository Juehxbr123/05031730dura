...
function sendBalanceToWs(ws, userId) {
  ...
  send(ws, { type: "balance", balances: getBalances(userId), walletAddress: row.walletAddress || "" });
}
...
const server = http.createServer((req, res) => {
  if (req.method === "GET" && req.url === "/tonconnect-manifest.json") {
    try {
      const txt = fs.readFileSync(path.join(__dirname, "tonconnect-manifest.json"), "utf8");
